...
This module implements a **Passkey Authenticator** for the [BTSG Account Trait](https://github.com/btsg-auth/docs) and [x/smart-account module](https://github.com/terpnetwork/terp-core/blob/main/x/smart-account/README.md). It enables passwordless, phishing-resistant authentication using **WebAuthn/FIDO2 passkeys** (e.g., device-bound keys like Touch ID or YubiKeys) in CosmWasm contracts. Passkeys provide cryptographic credentials that can be verified on-chain without revealing secrets.
...
| `QueryMsg` | `QueryMsg` |
| `SudoMsg` | `btsg_auth::AuthSudoMsg` (routes to hooks) |
